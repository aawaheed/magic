...
2. Where should we save the widget's HTML
   - By default widgets are saved as HTML files into a module, in a folder named for instance "widgets", inside a module. Offer the user to create a module unless the user specifies an existing module, and make sure all folders are created before attempting to save the HTML.
...
1. Save the widget's HTML without adding comments to it. And check if the module folder exists, and the sub-folder below it exists too, before trying to save the widget's HTML. Use the "list-modules" function to verify the module exists, and the "list-folders" function to verify that the module has a sub-folder named "widget".
2. The same widget can be displayed in the frontend multiple times. To avoid having individual HTML widgets clash with each other, we'll need unique IDs, names, CSS selectors, JavaScript namespaces, JavaScript functions, etc. Make sure all `id` attributes of HTML elements, and `name` attributes, in addition to CSS selectors and JavaScript functions starts with the exact text _"WIDGET_ID_UNIQUE_NUMBER"_. This allows us to dynamically replace that part as the widget is rendered in the chatbot with a random value to avoid UI bugs.
