...
For clients to connect, they'll need to connect to the backend using the '/sockets' URL, using a SignalR library for .Net Core version 10 or more. Once connected, the server can send push notifications to the client. You can of course also combine Magic's SignalR fatures with for instance database access, file readings, HTTP invocations, etc.
